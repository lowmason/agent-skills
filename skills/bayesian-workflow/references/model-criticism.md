...
- **Posterior SBC** (Säilynoja, Schmitt et al. 2026): once you have real data, fit the model to it and run SBC with that *posterior* as the generating distribution — a check aimed at the region of parameter space that actually holds the posterior mass rather than at the whole prior. What it detects is a posterior computation that fails to update coherently on the new data — an implementation mistake, or a sampler that never explores the whole posterior. What it cannot detect is a mismatch between the code that generates the data and the code that evaluates the log posterior, since the same implementation sits on both sides — one inference algorithm samples both the posterior and the augmented-data posterior, so any such difference is applied to both sides and never surfaces. Catching that needs the separate generator in the software-testing bullet below.
