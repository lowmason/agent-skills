...
into an unrequested refactor. This as a "clean-code family" grounded in Robert
C. Martin's *Clean Code* rule catalog.

## Goal

Ship a proactive-cleanup complement to `tech-debt`, 
clean-code family to this repo's stack (Polars / NumPyro / JAX / BLS-ETL) and
conventions, with honest provenance to both Martin and ytran14.
...
- **G1** — two skills (behavior + one reference with `references/`). Respects `writing-skills`'
...
- **Frontmatter:** `name: clean-coder`; a single dense `description` starting
  ...
  **Drop** non-standard `when_to_use:` block — fold its phrases into the
...
- `NOTICE` gets a **new, dedicated block** (not folded into "Lowell's originals"):
  adapted from rule catalog concept from
...
5. `NOTICE`, `.claude/CLAUDE.md`, and `README.md` are consistent
