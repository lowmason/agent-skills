...
- **My original skills** — `develop-testing-strategy`, `validate-data`, `explore-data`, `tech-debt`, `design-architecture`, `bls-data-context`, `recommend-probabilistic-model`, `recommend-visualization`, and `track-model-experiments` are my own work, MIT licensed (see [`LICENSE`](LICENSE)).
